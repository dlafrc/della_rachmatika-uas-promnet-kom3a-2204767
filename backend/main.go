...
func main() {
	...
	router.HandleFunc("/inventory", GetInventoryItems).Methods("GET")
	router.HandleFunc("/inventory", CreateInventoryItem).Methods("POST")
	router.HandleFunc("/inventory/{id}", GetInventoryItem).Methods("GET")
	router.HandleFunc("/inventory/{id}", UpdateInventoryItem).Methods("PUT")
	router.HandleFunc("/inventory/{id}", DeleteInventoryItem).Methods("DELETE")
	router.HandleFunc("/inventory/search", SearchInventoryByName).Methods("GET")
...
func GetInventoryItems(w http.ResponseWriter, r *http.Request) {
	...
	result, err := DB.Query("SELECT id, nama_barang, jumlah, harga_satuan, lokasi, deskripsi FROM inventory_della")
	if err != nil {
		panic(err.Error())
	}
	defer result.Close()
...
func CreateInventoryItem(w http.ResponseWriter, r *http.Request) {
	w.Header().Set("Content-Type", "application/json")
	stmt, err := DB.Prepare("INSERT INTO inventory_della(nama_barang, jumlah, harga_satuan, lokasi, deskripsi) VALUES(?,?,?,?,?)")
	if err != nil {
		panic(err.Error())
	}
...
func GetInventoryItem(w http.ResponseWriter, r *http.Request) {
	w.Header().Set("Content-Type", "application/json")
	params := mux.Vars(r)
	result, err := DB.Query("SELECT id, nama_barang, jumlah, harga_satuan, lokasi, deskripsi FROM inventory_della WHERE id = ?", params["id"])
	if err != nil {
		panic(err.Error())
	}
	defer result.Close()
...
func UpdateInventoryItem(w http.ResponseWriter, r *http.Request) {
	w.Header().Set("Content-Type", "application/json")
	params := mux.Vars(r)
	stmt, err := DB.Prepare("UPDATE inventory_della SET nama_barang = ?," +
...
func DeleteInventoryItem(w http.ResponseWriter, r *http.Request) {
	...
	stmt, err := DB.Prepare("DELETE FROM inventory_della WHERE id = ?")
	if err != nil {
		panic(err.Error())
	}
...
func SearchInventoryByName(w http.ResponseWriter, r *http.Request) {
	...
	fmt.Println("Received search term:", searchTerm) // Add this line for logging

	result, err := DB.Query("SELECT id, nama_barang, jumlah, harga_satuan, lokasi, deskripsi FROM inventory_della WHERE nama_barang LIKE ?", "%"+searchTerm+"%")
	if err != nil {
...
func InitDB() {
	DB, err = sql.Open("mysql", "root:@/db_2204767_dellarachmatikani_uas")
	if err != nil {
		panic(err)
	}
